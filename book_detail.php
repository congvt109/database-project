...
<div class="container">
  <a href="javascript:window.history.go(-1)" class="fa fa-undo"
    style="font-size: 1.5rem; margin: 10px; text-decoration: none; color: #434343; "> Quay lại</a>
  <hr>
  <div class="card">
    <div class="row">
        ...
        <article class="card-body p-5">
          ...
          <p class="author-detail-wrap" style="margin-top:20px;">
            ...
            <span class="category-info  h3 ">
              ...
              <a class="category-info"
                href="book.php?author_id=<?= $author['author_id']?>"><?= $author['name'].","?></a>
              <?php }
          ...
          <p class="publisher-detail-wrap">
            ...
            <span class="category-info  h3 ">
              ...
              <a class="category-info"
                href="book.php?publisher_id=<?= $publisher['publisher_id']?>"><?= $publisher['name']?></a>
              <?php } ?>
            </span>
          ...
          <p class="price-detail-wrap">
            ...
            <span class="category-info price h3 ">
              ...
              <span
                style="color:black;font-size: 1.2rem;text-decoration: line-through;"><?=number_format($book['price'], 0, ",", ".") ?>đ</span>
            </span>
          </p> <!-- price-detail-wrap .// -->
          ...
          <p class="discount-detail-wrap">
            ...
            <span class="category-info price h5 ">
              <span class="num"><?= number_format($book['discount'], 0, ",", ".") ?></span><span class="currency">
                VNĐ</span><span>( <?=ceil( $book['discount']/$book['price']*100 )?>% )</span>
            </span>
                                ...
                                if (isset($_GET['action']) && $_GET['action'] == 'delfvr')      //thực hiện xóa 
                                {
                                    ...
                                    if (isset($_GET['action']) && $_GET['action'] == 'addfvr') 
                                    {
                                        ...
                                        VALUES ('$bo_id', '$u_id')");
                                            ...
                                            $u_id = $currentUser['id'];
                                            ?>
          <form action="./book_detail.php?action=addfvr&id=<?=$bo_id?>" method="Post" enctype="multipart/form-data"
          ...
          <?php
                                    ...
                                    // <div >Hiện lại form yêu thích                                     
                                        ...
                                        $u_id = $currentUser['id'];
                                        ?>
          <form action="./book_detail.php?action=delfvr&id=<?=$bo_id?>" method="Post" enctype="multipart/form-data"
            autocomplete="off" id="registrationForm">
            <input type="hidden" name="book_id" id="book_id" value="<?= $bo_id ?>" />
            <input type="hidden" name="user_id" id="user_id" value="<?= $u_id?>" />
            <div class=" form-group">
            ...
            <div class="grid-3-col">
              <?php
                            ...
                            while ($genres = mysqli_fetch_array($result3)){ 
                                ...
                                ?>
              <a href="book.php?genres_id=<?= $genres['genres_id']?>">
                <h4 class="tag-content"><?= $genres['name'] ?> </h4>
              </a>
              <?php }
            ...
            <div class="col-sm-6">
              ...
              <div class="amount-selection-container">
                ...
                <form id="add-to-cart-form" action="cart.php?action=add&id=<?=$book['id']?>" method="POST">
                  <div class="category row"><input class="number-select" type="number" value="1"
                      name="quantity[<?=$book['id']?>]" size="2" /></div>
              </div>
              ...
              <button data-id="<?=$book['id']?>" class="btn-add-to-cart btn btn-lg btn-outline-primary text-uppercase"
  ...
  <div class="review_card">
    <div class="row d-flex">
      <div class=""> <img class="profile-pic"
          src="./<?=  isset($row_comment['avatar']) ? $row_comment['avatar'] : "assets/image/user/user.png"?>" alt=""
          style="width: 40px;height:40px;border-radius: 100%;"> </div>
  ...
  <div class="swiper featured-slider">
    ...
    <div class="swiper-wrapper">
      ...
      <div class="swiper-slide box">
        ...
        <div class="image">
          <a href="book_detail.php?id=<?= $row_genres['id'] ?>"><img src="./<?= $row_genres['image'] ?>"
              alt="<?= $row_genres['tittle'] ?>" alt=""></a>
        </div>
        <div class="content">
          <h3><?= $row_genres['tittle'] ?></h3>
          <div class="price"><?= number_format($row_genres['price']-$row_genres['discount'], 0, ",", ".") ?>đ <span>
              <?= number_format($row_genres['price'], 0, ",", ".") ?></span></div>
          <button data-id="<?=$row_genres['id']?>" class="btn-add-to-cart btn btn-lg btn-outline-primary text-uppercase"
            style="background-color:#f59f00;color:#fff">
            <i class="fas fa-shopping-cart"></i> Thêm vào giỏ hàng </button>
        </div>
      </div>
  ...
  <div class="swiper featured-slider">
    ...
    <div class="swiper-wrapper">
      ...
      <div class="swiper-slide box">
        ...
        <div class="image">
          <a href="book_detail.php?id=<?= $row_author['id'] ?>"><img src="./<?= $row_author['image'] ?>"
              alt="<?= $row_author['tittle'] ?>" alt=""></a>
        </div>
        <div class="content">
          <h3><?= $row_author['tittle'] ?></h3>
          <div class="price"><?= number_format($row_author['price']-$row_author['discount'], 0, ",", ".") ?>đ <span>
              <?= number_format($row_author['price'], 0, ",", ".") ?></span></div>
          <button data-id="<?=$row_author['id']?>" class="btn-add-to-cart btn btn-lg btn-outline-primary text-uppercase"
            style="background-color:#f59f00;color:#fff">
            <i class="fas fa-shopping-cart"></i> Thêm vào giỏ hàng </button>
        </div>
      </div>
