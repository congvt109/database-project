...
<div class="container">
        ...
        <ul class="navbar-nav d-lg-flex align-items-lg-center">
          <li class="nav-item active" style="border:1px solid #777;margin-left:0.5rem">
            <form id="book-search" method="GET">
              ...
              <input style="width:300px;" type="text" value="<?= isset($_GET['tittle']) ? $_GET['tittle'] : "" ?>"
                name="tittle" placeholder="Nhập tên sách" />
          ...
          <li class="nav-item active" style="border:1px solid #777;margin-left:0.5rem">
            <select name="sort" id="sort"
              ...
              <option <?php if (isset($_GET['sort']) && $_GET['field'] == "price" && $_GET['sort'] == "desc") { ?>
                selected <?php } ?> value="?<?= $sortParam ?>field=price&sort=desc">Giá giảm dần</option>

              <!-- Neu TON TAI sortParam(1 str khac cua search) thi se KET HOP ca dk search va order -->
              <option <?php if ((isset($_GET['sort']) && $_GET['field']) == "price" && $_GET['sort'] == "asc") { ?>
                selected <?php } ?> value="?<?= $sortParam ?>field=price&sort=asc">Giá tăng dần</option>

              <option
                <?php if (isset($_GET['sort']) && $_GET['field'] == "created_date" && $_GET['sort'] == "desc") { ?>
                selected <?php } ?> value="?<?= $sortParam ?>field=created_date&sort=desc">Mới nhất</option>

              <!-- Neu TON TAI sortParam(1 str khac cua search) thi se KET HOP ca dk search va order -->
              <option
                <?php if (isset($_GET['sort']) && ($_GET['field']) == "created_date" && $_GET['sort'] == "asc") { ?>
                selected <?php } ?> value="?<?= $sortParam ?>field=created_date&sort=asc">Cũ nhất</option>
            </select>
          </li>
    ...
    <div class="ml-auto mt-3 mr-2">
      <nav aria-label="Page navigation example">
        ...
        <ul class="pagination">
          ...
          <a class="page-item" aria-label="Previous"
            href="?<?= $param ?>per_page=<?= $item_per_page ?>&page=<?= $prev_page ?>"><span aria-hidden="true"
              class="font-weight-bold">&lt;</span> <span class="sr-only">Previous</span></a>
          <?php }
          ...
          <a class="page-item" href="?<?= $param ?>per_page=<?= $item_per_page ?>&page=<?= $next_page ?>"><span
              aria-hidden="true" class="font-weight-bold">&gt;</span> <span class="sr-only">Next</span></a>
          <?php }
  ...
  <div id="content" class="my-5">
    ...
    <div id="filterbar" class="collapse">
      ...
      <div class="box border-bottom">
        <div class="box-label text-uppercase d-flex align-items-center">Thể loại<button class="btn ml-auto"
      ...
      <div class="box border-bottom">
        <div class="box-label text-uppercase d-flex align-items-center">Nhà xuất bản<button class="btn ml-auto"
      ...
      <div class="box border-bottom">
        <div class="box-label text-uppercase d-flex align-items-center">Tác giả<button class="btn ml-auto" type="button"
            data-toggle="collapse" data-target="#inner-box3" aria-expanded="false" aria-controls="inner-box3"><span
      ...
      <div class="box">
        <div class="box-label text-uppercase d-flex align-items-center">Giá<button class="btn ml-auto" type="button"
            data-toggle="collapse" data-target="#size" aria-expanded="false" aria-controls="size"><span
              class="fas fa-plus"></span></button> </div>
        <div id="size" class="collapse">
          <div class="btn-group d-flex align-items-center flex-wrap">
            <label><a href="./book.php?sort_price=50000">
                < 50.000</a></label>
            <label><a href="./book.php?sort_price=100000">
                < 100.000</a></label>
            <label><a href="./book.php?sort_price=150000">
                < 150.000</a></label>
            <label><a href="./book.php?sort_price=200000">
                < 200.000</a></label>
            <label><a href="./book.php?sort_price=300000">
                < 300.000</a> </label>
            <label><a href="./book.php?sort_price=400000">
                < 400.000</a> </label>
            <label><a href="./book.php?sort_price=500000">
                < 500.000</a> </label>
            <label><a href="./book.php?sort_price=1000000">
                < 1.000.000</a> </label>
          ...
          <div class="row mx-0">
            ...
            <div class="col-lg-3 col-md-6 pt-md-4 pt-3">
              <div class="card d-flex flex-column">
                <a href="book_detail.php?id=<?= $row['id'] ?>" class="card-img"> <img src="<?= $row['image'] ?>" alt="">
                </a>
                <a href="book_detail.php?id=<?= $row['id'] ?>" class="product-name"
                  style="font-size:1.4rem;"><?= $row['tittle'] ?></a>
                <div class="card-info" style="margin-top: 0.5rem;">
                  ...
                  <div class="d-flex price">
                    <p class="price" style="margin-top: 0.6rem; font-weight: 600; font-size: 20px;color: #f33f3f;">
                      <?= number_format($row['price'] - $row['discount'], 0, ",", ".") ?>đ<span><?= number_format($row['price'], 0, ",", ".") ?>đ</span>
                    </p>
                  </div>
                </div>
              </div>
